    ...
    return {
        ...
        matches(trace) {
            if (filter.type && !filter.type.includes(trace.type))
                return false;
            if (filter.callFrom && trace.type === 'call' && !filter.callFrom.includes(trace.action.from.toLowerCase()))
                return false;
            if (filter.callTo && trace.type === 'call' && !filter.callTo.includes(trace.action.to.toLowerCase()))
                return false;
            // `sighash` is absent on calls with less than 4 bytes of input.
            if (filter.callSighash &&
                trace.type === 'call' &&
