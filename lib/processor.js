...
const createEvmBatchProcessor = (config, options) => {
    ...
    const processor = new evm_processor_1.EvmBatchProcessor()
        ...
        .setFields(options?.fields ? options?.fields : DEFAULT_FIELDS);
...
const run = async ({ fromNow, chainId = 1, stateSchema, processors, postProcessors, validators, postValidation, fields }) => {
    if (!fromNow) {
