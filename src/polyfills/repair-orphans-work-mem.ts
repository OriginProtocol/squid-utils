...
/** Minimal shape of what `repairOrphans` is handed — typeorm's transactional `EntityManager`. */
...
const resolveWorkMem = (): string => {
  const configured = process.env.SQUID_REPAIR_ORPHANS_WORK_MEM
  if (!configured) return DEFAULT_WORK_MEM
  if (!WORK_MEM_PATTERN.test(configured)) {
    console.warn(
      `SQUID_REPAIR_ORPHANS_WORK_MEM=${JSON.stringify(configured)} is not a Postgres memory value ` +
 ...
 */
export const withRepairOrphansWorkMem = <T extends TypeormDatabase>(db: T): T => {
  const self = db as any
  ...
  if (typeof self.repairOrphans !== 'function') {
    throw new Error(
      'Unable to raise work_mem for repairOrphans: TypeormDatabase.prototype.repairOrphans is missing. ' +
        'Check whether @subsquid/typeorm-store still needs this polyfill.',
    )
  }
  if (Object.prototype.hasOwnProperty.call(self, 'repairOrphans')) return db

  const original = self.repairOrphans as (em: EntityManagerLike) => Promise<void>
  const workMem = resolveWorkMem()

  self.repairOrphans = async function (em: EntityManagerLike): Promise<void> {
    try {
 ...
 */
export const createTypeormDatabase = (
  ...args: ConstructorParameters<typeof TypeormDatabase>
): TypeormDatabase => withRepairOrphansWorkMem(new TypeormDatabase(...args))
